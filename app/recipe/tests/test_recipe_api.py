...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_recipe_other_users_recipe_error(self):
        new_user = create_user(
            email = 'user3@example.com', password = 'test@123'
        )
    ...
    def test_create_recipe_with_existing_ingredient(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        reciepes = Recipe.objects.filter(user=self.user)
        self.assertEqual(reciepes.count(), 1)
        recipe = reciepes[0]
        self.assertEqual(recipe.ingredients.count(), 2)
    ...
    def test_update_recipe_assign_ingredient(self):
        ingredient1 = Ingredient.objects.create(user=self.user, name='White Pepper')
        recipe = create_recipe(user=self.user)
    ...
    def test_filter_recipes_by_tags(self):
        ...
        self.assertNotIn(recipe3.title, [recipe['title'] for recipe in res.data])
    
    def test_filter_recipes_by_ingredients(self):
        recipe1 = create_recipe(user=self.user, title='Pasta Primavera')
        recipe2 = create_recipe(user=self.user, title='Chicken Alfredo')
        recipe3 = create_recipe(user=self.user, title='Caesar Salad')

        ingredient1 = Ingredient.objects.create(user=self.user, name='Pasta')
